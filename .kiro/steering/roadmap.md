...
- [ ] ~~Correct the manifest status vocabulary~~ — **retracted, this was wrong.** Both forms genuinely exist and are distinct: `failed_chunk_{n}` (singular) is written by the *synthesis* path (`pdf_processor.py:1522`), `failed_chunks` (plural, with a `failed_chunks` list) by the *extraction-chunk* path (`pdf_processor.py:1132`), alongside `failed_qc_pipeline` (`orchestrator.py:130`), `failed_validation` (`pdf_processor.py:678`) and `failed_schema_validation` (`pdf_processor.py:970`). `src/pipeline/README.md:57` already documents both. `risk-remediation` Requirement 5.3 is therefore **correct** as written. The real defect in this area is that the two stages report failure in *different shapes* (`error` string vs `failed_chunks` list plus metadata), which is what Requirement 5.4 targets.
