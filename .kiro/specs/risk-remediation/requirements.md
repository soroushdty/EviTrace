...
| C4 / R4 | **PARTIALLY VALID** | The defect is real but **currently unreachable**: `ocr_derived` is never set `True` anywhere in `src/`, so the FragmentSelector branch runs only in hand-built test records. Fixing C4 alone changes nothing observable — `ocr_derived` propagation must be fixed too, and that is in none of C1–C10. The legacy fix also does not compile: `_compute_sentence_to_char_range` receives bare strings, with no block or page in scope. **Larger bug the audit missed**: the char-range map is built from *paragraph* texts but looked up by *sentence* texts, so most native annotations silently get `{"start": 0, "end": 0}`. That defeats the proposed bbox retrieval too and affects the majority path today. |
