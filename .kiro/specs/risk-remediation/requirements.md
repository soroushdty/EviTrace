...
1. When an evidence package is built using default configuration for a paper whose substantive text is 30,000 characters, the evidence index shall select at least 60 percent of that substantive text. **⚠ VALIDATION 2026-09-22: this criterion is NOT met at the shipped defaults and is unsatisfiable as worded while any finite item cap exists — measured 0.15–0.50 for short-sentence papers. Decision pending; see `validation-report.md` §1.** *Substantive text* is defined as the total character length of all evidence items derived from the document's TEI (sentences, figure captions, and tables) before ranking or pruning, excluding items attributed to the document metadata section; the selected share is measured in the same units.
