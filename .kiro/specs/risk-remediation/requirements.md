...
**Gap analysis (2026-09-21, `research.md`).** All appendix claims were re-verified against `a57294d`, at runtime where behavioural, and Requirement 7 was probed against live GROBID 0.8.2 output. Four consequences are folded into the requirements below: Requirement 8 is **implemented**; Requirement 7's acceptance criteria are reworded because GROBID expresses no figure containment and the legacy fix is now **proven** to emit zero figure/table evidence on real output; Requirement 4 gains an explicit sentence-source criterion because OCR text never reaches the semantic layer today; and a new **Requirement 11** covers three coordinate defects discovered during the probe. Requirement 10 now defines its denominator. Full verdict table and evidence: `research.md`.

## Boundary Context

- **In scope**: Eleven defects in already-shipped behavior — final-output schema validation, adjudication-to-reconciliation wiring, annotation identifier determinism, OCR provenance propagation and annotation region accuracy, synthesis-stage repair parity, page classification on the cached-parse path, figure/table section attribution, quality-control base-class enforcement, extractor naming in agreement and adjudication outputs, evidence coverage per chunk, and evidence page coordinates from GROBID.
...
3. When a sentence is derived from a block that came from a native page, the pipeline shall mark that sentence as not OCR-derived, and character-offset behavior for such sentences shall not regress (the existing behavior is itself defective — the offset map is keyed by paragraph text and looked up by sentence text, and hard-codes page 0 — and may be improved).
...
9. When a page is classified as scanned and OCR text exists for it, the pipeline shall derive sentences for that page from the OCR blocks, so that OCR-derived sentences exist to be marked. *(Added 2026-09-21: today sentences are produced only from the GROBID artifact, so criterion 2 has no code path without this.)*
...
4. The synthesis stage and the extraction-chunk stages shall expose the same retry limits and the same failure reporting shape to an operator. The repair-attempt limit shall be an operator-configurable key in the `retry` section of the configuration (today it is a hard-coded constructor default with no configuration surface), registered per the configuration steering rules.
...
*Reworded 2026-09-21. Live GROBID 0.8.2 output (three papers, 35 figures) shows that no `<figure>` is ever a descendant of a `<div>`; all figures and tables are `<body>` siblings emitted after the last section, so "the section that contains it" is not expressible in the TEI. The legacy fix (`div.findall("./figure")`) is therefore **proven wrong** — it emits zero figure/table evidence on real output while passing both in-repo fixtures. The observable defect is that every figure and table inherits the heading of the last body section. The reworded criteria use the only containment signal GROBID provides: in-text citations (`<ref type="figure">`, `<ref type="table">`).*
...
1. When a figure caption is indexed as evidence, the evidence index shall attribute it to the heading of the first body section, in document order, that cites the figure; where no section cites it, the fallback in criterion 4 shall apply.
2. When a table is indexed as evidence, the evidence index shall attribute it to the heading of the first body section, in document order, that cites the table; where no section cites it, the fallback in criterion 4 shall apply.
...
4. Where a section has no heading, or a figure or table is cited by no section, the evidence index shall attribute the item to a single documented neutral label carrying no section-score boost or penalty; section attribution shall be determined per section and never carried over from a preceding section.
...
*Implemented 2026-09-21 (`src/quality_control/models.py`: `ABC` added to `QualityMetrics`, `InterRaterMetrics`, `AdjudicationRules`; regression tests in `tests/src/quality_control/test_qc_models.py`; `CHANGELOG.md` entry). Retained here so traceability is complete; design and tasks should record it as done rather than re-plan it.*
...
1. When an evidence package is built using default configuration for the reference paper — the bioRxiv GROBID fixture `tests/fixtures/grobid_tei/biorxiv_2020.03.24.004655.tei.xml` (≈31,000 characters of substantive text) — the evidence index shall select at least 60 percent of that substantive text. For other papers the 60 percent floor is an expectation that is monitored, not a guarantee: a paper whose highest-ranked evidence items are short falls below it, and criterion 4 records that shortfall. *Substantive text* is defined as the total character length of all evidence items derived from the document's TEI (sentences, figure captions, and tables) before ranking or pruning, excluding items attributed to the document metadata section; the selected share is measured in the same units. *Reworded 2026-09-23 by spec-owner decision (option (a) in `validation-report.md` §1). The original wording — "for a paper whose substantive text is 30,000 characters" — was unsatisfiable while any finite item cap exists: validation measured 0.15–0.50 for short-sentence papers at the shipped defaults (150 items / 30,000 chars). A sentence-length premise was considered and rejected because selection ranks by relevance, not length: a 30,000-character paper whose items average exactly 120 characters measured 0.40 on 2026-09-23 when its short items outranked its long ones (pinned by `test_mean_item_length_does_not_guarantee_the_floor`).*
...
### Requirement 11: Evidence Page Coordinates from GROBID

*Added 2026-09-21 from the gap analysis. Three independent defects mean no evidence item carries a page today: (a) the GROBID request sends `teiCoordinates` as one comma-joined value, which GROBID treats as a single unknown element name and so emits no `coords` attributes at all; (b) both coordinate parsers expect `page;x0,y0,x1,y1` while GROBID emits `page,x,y,w,h` boxes separated by `;`, so they return nothing on real input; (c) sentence elements never carry coordinates and the evidence index reads the sentence's own attribute rather than inheriting from its paragraph. In-repo fixtures use the wrong format and therefore never caught this.*

**Objective:** As a reviewer opening an evidence citation, I want the evidence item to name the page it came from, so that I can locate it in the PDF instead of searching the whole document.

#### Acceptance Criteria

1. When the pipeline requests a GROBID full-text parse, it shall request coordinates in the form GROBID accepts, so that the returned TEI carries coordinate attributes for the requested element types.
2. When a TEI coordinate attribute in GROBID's emitted format is parsed, the parser shall return the page number and bounding box of the first box, and shall not return an absent result for well-formed input.
3. When a sentence element carries no coordinates but its enclosing paragraph does, the evidence index shall attribute the paragraph's page to the sentence.
4. When a figure or table element carries coordinates, its evidence item shall carry that page.
5. If coordinates are absent or malformed for an element, then the evidence index shall record the page as unknown and shall continue indexing the remaining items.
6. The test fixtures for TEI parsing shall use GROBID's actual coordinate format.
...
| C7 / R7 | **VALID, reworded 2026-09-21** | Confirmed: figure/table loops sit after the div loop and read the leaked final `section_path`; `section_path` is loop-carried so a head-less div inherits the previous heading. **Superseded by the live probe (`research.md`)**: the "trailing References" sink cannot occur because GROBID places references, acknowledgements and annexes in `<back>`, never `<body>`; the real failure is that 100 % of figure/table items inherit the *last body heading* (observed: a Methods sub-head, `Scenario`, `Conclusion`). The legacy fix is **proven wrong**: 0 of 35 real figures are inside a `<div>`. Additional defect found: every GROBID table produces **two** items (caption via `.//figure`, rows via `.//table`), and the row item's xpath is the non-unique `.//table`. Attribution must use `<ref target="#fig_N">` citations with a neutral fallback. |
| C8 / R8 | **IMPLEMENTED 2026-09-21** | Confirmed at runtime and fixed: `ABC` added to the three bases; the nine bare instantiations in `test_domain_agnosticism.py` became test-local stub subclasses; seven regression tests added; full suite green (1477 passed). |
