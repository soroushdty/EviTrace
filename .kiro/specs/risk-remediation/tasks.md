...
> **All 34 sub-tasks are complete. Feature validation returned NO-GO on Requirement 10.1; the
> spec owner resolved it on 2026-09-23 by rewording 10.1 to name the reference bioRxiv paper
> (option (a)). Non-blocking drifts and open questions remain in `validation-report.md`.**
...
- [x] 7.1 (P) Request coordinates in the form GROBID accepts and parse its real grammar
...
- [x] 7.4 Cross-agreement and fixture tests for coordinates
...
- [x] 8.1 Attribute figures and tables to the first citing section, one item each, with real pages
...
- [x] 9.2 Record the coverage outcome per paper
...
- [x] 10.1 Extend the repair loop with the synthesis-only inputs
...
- [x] 10.2 Route the synthesis stage through the repair loop with the configured attempt limit
...
- [x] 10.3 Record failures with one manifest shape for chunks, synthesis, and the output write
...
- [x] 10.4 Test synthesis repair, failure records, and the final-output negative path
...
- [x] 11.1 Unify branch construction into one function used by the cache-miss path
...
- [x] 11.2 Persist per-page classification beside the TEI cache and route the cache-hit path through the unified function
...
- [x] 12.2 Migration bug-condition test file
...
- [x] 12.3 Migration preservation test file
...
- [x] 12.4 Full verification and changelog
...
- 10.1: **accepted deviation** from the task's "Done when" literal text — `protected_evidence_ids` is forwarded to the INITIAL budget check only, not the repair-attempt check: the repair check passes the plain-text repair prompt as `evidence_text`, and with protected ids `_check_and_mitigate_budget` hard-raises `TokenBudgetExceededError` instead of mitigating (verified in-process by the reviewer). Forwarding there would make synthesis stricter than chunks (against 5.4). The initial `extract_chunk` now passes `stage` explicitly (default "extraction_chunk" — same label `_default_extract_stage` derived). Pre-existing: `test_repair_retry.py` cannot be collected in isolation (same MagicMock ordering as test_pdf_processor_helpers.py).
- 10.2: one `RepairRetryLoop` per paper (limit from `openai_config.get("max_repair_attempts", 2)`) shared via `_run_parallel_chunks(..., repair_loop=)`; its `None` fallback builds a default-2 loop and is unreachable from `process_pdf`. `process_pdf` no longer imports `extract_chunk` at entry (lazy inside the loop). `src/pipeline/README.md:35` diagram still says `extract_chunk (synthesis chunk)` — update at 10.3.
- 10.3: manifest `error` for synthesis exhaustion is now the underlying validator message (`failures[-1]["last_error"]`, per design) — `str(RepairExhaustedError)` details live in the record's `error_type`/`attempts`; no src/test consumer parsed the old string. `except Exception` around the output gate also labels normalizer/validator exceptions `failed_output_write` (error_type keeps the real class). `_record_failure` calls `save_manifest` itself; callers hold the lock. `EXPECTED_STATUSES` extension is 10.4's. Pre-existing isolated-collection issue of the helpers/repair test files → 12.4 / testing.md note.
...
- 12.2: 16 sub-checks, all independently verified to fail at `a57294d` and pass now. The 11.2 check exercises `evidence_index._parse_coords` + an end-to-end page assertion rather than design's `parse_tei_coords` (dormant on the production path per 7.1) — intentional substitution. The 10.1 check asserts coverage is *measured* and ≥ 0.6 (the API's absence is the pre-change failure); the ratio itself is the same on both trees. The 5.1 call-site regex guard gives a false failure on single-quoted `stage='synthesis'` or 3-level paren nesting before the kwarg (the functional sibling test covers 5.1 substantively). Evidence caps hardcoded 150/30000 (task 1.1's config test pins the same pair).
- 12.3: 14 checks green on both `a57294d` and the current tree; each killed by a targeted mutation (reviewer applied 14). Necessary scoping: selector test pins TYPES/order/quote/ids but not offsets (0,0 pre-change vs real now — 4.3 permits the improvement); sentence comparison excludes the new `source` key; `failed_chunks` assertions cover the old keys only (entry is a superset). Minor: the coverage-report test passes an inert `grobid_vs_native_ratio_threshold` key (real key is `extraction_coverage_ratio_threshold`) so that one 0.6 comes from the code default; `.kiro/steering/testing.md:219` says migration sub-checks use `pytest.fail()` but both preservation files use plain asserts — sync at 12.4.
- 12.4: full suite incl. slow **1920 passed / 3 skipped** (pre-spec baseline 1470 / 2); slow-only 22 passed; dependency directions 9; config consistency 17; `main.py --help` OK. Doc syncs applied: `testing.md` (root conftest also appends the repo root; `pytest.fail()` is the bug-condition convention, preservation files use plain asserts), `CLAUDE.md` + `product.md` (`project()` also reads the scalar `unified.document_id`), root `README.md` (two new env vars), `configs/config.yaml` (`tei_cache_dir` documents the `.pages.json` sidecar). CHANGELOG entry covers the behaviour changes operators need (new annotation ids, OCR on cache hits, per-page QC metrics, stale evidence caches, config defaults, manifest keys, GROBID coords).
- 12.4 (validation): open questions left deliberately unresolved, both recorded for the roadmap rather than patched: (a) adjudication `confidence` semantics — with distinct extractor names the built-in formula yields `<= 1/N` and ties break by index order; no requirement governs it and no `src/` consumer branches on it (`test_builtin_impls_identity.py:71` pins `1/3`); (b) `content["segments"]` / `content["pages"]` remain primary-only while `exact_text` includes used OCR blocks — verified inert: `content["segments"]` has zero consumers in `src/`.
- 12.4 (boundary audit): no violations. The four JSON schemas, `src/agents/` (prompt-cache prefix), `scan_detector.py` and the evidence-cache format are byte-identical across `a57294d..HEAD`; `models.py`'s entire diff is the four-line R8 `ABC` change (no dataclass field touched). Changed-file set matches the File Structure Plan exactly (15 files, none unplanned, none planned-but-untouched). Single new cross-package import is the designed `pipeline → pdf_extractor.parse_tei_coords`. Pre-existing dormant imports confirmed identical at `a57294d` and left alone: `parse_grobid_tei` in `extraction_pipeline.py:31` (its `_parse_coords` block path is unused in production — production reads coords via `evidence_index._parse_coords` and `quality_control._page_from_tei_coords`) and `build_chunk_evidence_package` in `pdf_processor.py:14`. Roadmap candidates, not spec defects.
- 12.4 (coverage audit): all 55 acceptance criteria have implementing code AND a killing test — **0 gaps, 0 untested** (verified by ~33 mutations across two validators). Seven criteria carry wording drift or partial coverage; one is a genuine unmet criterion:
  - **10.1 UNMET as worded (blocking).** Independently measured at shipped defaults (150 items / 30 000 chars): a 30 000-char paper of 300x100-char sentences covers 0.500; 600x50 -> 0.250; 1000x30 -> 0.150. Only papers whose mean evidence item is >= 120 chars clear the 0.6 floor (reference fixtures do: biorxiv 0.946, plosone 0.699). Because the item cap bounds `selected_chars` at `150 x mean_item_len`, **no finite item cap can satisfy the criterion universally** — it is unsatisfiable as written. Fix is either (a) reword 10.1 to name the reference corpus / a sentence-length premise, or (b) drop the item cap and let the 30 000-char cap alone prune (10.5 stays satisfied — ranking and the char cap remain). Decision belongs to the spec owner.
  - 10.2 worded about the token budget but tested via the character cap; the two enforcement points are never spanned by one test.
  - 2.4 "auditable from the output alone" untrue for the orchestrated pipeline: provenance reaches disk only through the standalone extractor CLI; `outputs/<paper>.extracted.json` carries none.
  - 4.3 the 12.3 preservation test is deliberately tautological on offsets (real coverage lives in 5.3/12.1); a native offset miss now yields quote-only rather than `{0,0}` — permitted by "may be improved", unpinned.
  - 5.4 limit parity mutation-proven; failure-*shape* parity inferred (same builder, same constant) rather than compared side by side; the accepted `protected_evidence_ids` deviation makes synthesis asymmetric at the initial budget check and is untested.
  - 1.4 both concrete skip paths tested; the universal claim rests on a source-substring scan a comment can satisfy.
  - 3.4 no time/randomness, but `document_source` is filesystem-derived and the two entry points disagree (`.stem` vs `.name`) -> different ids for the same paper across entry points.
  - Sub-criterion 3.3: both hops guarded, no end-to-end link (the 12.1 fixture has no duplicated sentence text). 4.9's implementation predicate ("page has no primary text") is a proxy for "page classified scanned".
  - Dead code confirmed: disabling both caps in `build_chunk_evidence_package` breaks no test — it is imported but never called, so R10's per-chunk caps have no live consumer.
