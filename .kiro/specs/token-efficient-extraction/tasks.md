...
- `tests/src/utils/test_safe_logging_properties.py::test_truncated_log_includes_sha256_hash_and_respects_limit` fails on a clean checkout unrelated to this feature (confirmed by stashing all task 8.2 changes and reproducing identically). Hypothesis found a falsifying example at `max_chars=14` where the truncated-preview log message still contains the full untruncated data verbatim -- a genuine pre-existing bug in the safe-logging truncation boundary for very small `max_chars` values, unrelated to token-efficient-extraction. Disregard at checkpoint gates (tasks 9, 11) alongside the 3 manifest-resume failures, unless a new run introduces failures beyond these 4.
