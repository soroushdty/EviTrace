...
- **REQUIRES ESCALATION — Requirement 9.4 is not satisfied in production.** Task 10.1 discovered and independently-verified-by-review that `pdf_processor.py`'s real budget-mitigation integration (`_check_and_mitigate_budget`, task 8.2) forwards the actual evidence package straight into `token_budget.py`'s confidence-blind, flat-text `apply_mitigation()`/`_prune_evidence()` -- the same limitation tasks 3.1/3.2 already flagged as needing structured data "only available in pdf_processor.py's real integration." That integration never materialized: task 8.2 wired budget checks in but did not add confidence-aware pruning. Reproduced concretely: a high-confidence Evidence_ID sorted later in the package gets dropped under pruning while a low-confidence one sorted earlier survives -- the opposite of Req 9.4's "preserve high-confidence Evidence_ID references." This is a real production correctness gap (silently-vanishing evidence citations under token pressure), not just a test-coverage gap, and it is pinned (not silently passed) by `test_req_9_4_known_limitation_evidence_pruning_is_confidence_blind` in `test_token_efficiency_regression.py`. No task in this spec's decomposition can fix it without reopening task 8.2 or adding a new remediation task -- flagging for the user/spec-owner before this feature is considered done.
