...
- **Extends**: xtrace-toolkit — `R-X-2` (per-run reproducibility manifest: git commit, environment, seeds, resolved config, per-artifact hashes) is the same artifact as prov R9.2 and must not be shipped twice. **Corrected during cross-spec review**: ownership of `R-X-2` and `run_manifest.json` sits with `cost-and-run-reporting`, which ships in wave 1; this spec references that manifest and contributes only the per-artifact-file hashes. `R-GOV-1`'s append-only ledger is a projection of the core graph and is exported through this spec's serializers.
- **Adjacent**: cost-and-run-reporting (multiagent R23/R27 — owns cost content, this spec owns its placement in the package; also owns `R-X-2`/`run_manifest.json`, which this spec consumes by reference); corpus-and-schema-builder (multiagent R1–R2 per-document audit trails on batch upload — must share this container); multiagent-extraction and evidence-routing (produce stage outputs referenced by multiagent R22.2).
