...
- [x] 8. Integrate telemetry and budget into API client and pipeline
  ...
  - [x] 8.2 Update `src/pipeline/pdf_processor.py` to integrate deterministic merge, token budget, and repair telemetry
...
- [x] 10. Implement token-efficiency regression tests
  - [x] 10.1 Create `tests/src/pipeline/test_token_efficiency_regression.py` with fixture-based regression tests
...
- `tests/src/utils/test_safe_logging_properties.py::test_truncated_log_includes_sha256_hash_and_respects_limit` fails on a clean checkout unrelated to this feature (confirmed by stashing all task 8.2 changes and reproducing identically). Hypothesis found a falsifying example at `max_chars=14` where the truncated-preview log message still contains the full untruncated data verbatim -- a genuine pre-existing bug in the safe-logging truncation boundary for very small `max_chars` values, unrelated to token-efficient-extraction.
- `tests/src/text_processing/test_matchers_properties.py::test_lexical_matcher_embedded_needle_found` fails on a clean checkout unrelated to this feature (confirmed by stashing task 10.1's changes and reproducing identically on commit 53d732f). Hypothesis found a falsifying example (`base` containing a non-ASCII `'AΣA'` substring near the end, `needle_len=30`) where `LexicalMatcher.search()` returns `None` instead of finding the embedded needle -- a pre-existing bug in text_processing's lexical matcher, unrelated to token-efficient-extraction. Disregard at checkpoint gates (task 11) alongside the other 4 known failures, unless a new run introduces failures beyond these 5.
...
- **RESOLVED — Requirement 9.4 evidence-pruning gap (was: not satisfied in production).** `/kiro-validate-impl`'s feature-level gate caught that `pdf_processor.py`'s real budget-mitigation integration (`_check_and_mitigate_budget`, task 8.2) forwarded the actual synthesis-stage evidence package straight into `token_budget.py`'s confidence-blind, flat-text `apply_mitigation()`/`_prune_evidence()`, silently dropping high-confidence Evidence_IDs under token pressure. Fixed via a remediation round: `pdf_processor.py` gained `_collect_protected_evidence_ids()` (union of Evidence_IDs referenced by any confidence="h" entry across `merged_fields_filtered`/`prefilled_fields`/conflict candidates -- all only available at the synthesis call site, since extraction-chunk/repair pruning happens before any confidence data exists) and `_prune_evidence_json_preserving_protected()` (parses the real evidence JSON, prunes only unprotected items). `_check_and_mitigate_budget()`'s new `protected_evidence_ids` parameter defaults to `None`/empty for extraction-chunk and validation_repair call sites, which take the exact original `token_budget.apply_mitigation()` path, byte-identical to before. `token_budget.py` itself is untouched and remains generically confidence-blind by design -- the fix lives one layer above it, where confidence data actually exists. If protected evidence alone exceeds budget, raises `TokenBudgetExceededError` rather than silently truncating a protected citation. Verified via two independent adversarial reproductions (implementer + reviewer, each with their own constructed fixtures) through the real `process_pdf` synthesis path.
